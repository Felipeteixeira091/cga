...
if ($acao === "cadastroUSUARIO") {
    ...
    $gestao = $txtTitulo['gestao'];
    $coordenador = $txtTitulo['coordenador'];
    $cn = $txtTitulo['cn'];
    $acesso = $txtTitulo['acesso'];

    cadastroUSUARIO($mysqli, $re_sessao, $re, $nome, $estado, $email, $telefone, $cargo, $gestao, $coordenador, $cn, $acesso, $combustivel, $cartao, $data, $hora);
} else
if ($acao === "editaUSUARIO") {
    ...
    editaUSUARIO($mysqli, $re_sessao, $re, $nome, $estado, $email, $telefone, $cargo, $supervisor, $gestao, $cn, $acesso, $ativo, $senha, $data, $hora);
} else
...
{
    $sql = "select usr.re as re, usr.nome as nome, usr.estado as estado, usr.supervisor as coordenador, usr.cargo as cargo, usr.email as email, usr.telefone as telefone, usr.cn as cn, usr.sistema as sistema, usr.ativo as ativo, usr.gestao as gestao from usuario usr WHERE usr.re='{$re}'";
...
function dadosCartao($mysqli, $re, $re_resp)
{
    ...
    echo json_encode(array("cartao" => $cartao, "permissao1" => $p1, "permissao2" => $p2));
...
function dadosFrota($mysqli, $re, $re_resp)
{
    ...
    //Permissão para alterar KM
    $p = permissaoVerifica($mysqli, "17", $re_resp);

    echo json_encode(array("frota" => $frota, "permissao" => $p));
...
function estadoLista($mysqli, $re_sessao)
{
    ...
    // Listar todos os UF's do sistema
    $p = permissaoVerifica($mysqli, "86", $re_sessao);
...
function gestaoLista($mysqli)
{
    $sql = "select id, nome from gestao order by nome";
...
function cnLista($mysqli, $re_sessao)
{
    $p = permissaoVerifica($mysqli, "30", $re_sessao); //Lista CN (BR)

    $regiao = regiao($mysqli, $re_sessao)['regiao'];

    $w = "";
    if ($p === 0) {
        $w = " where regiao='{$regiao}'";
    }

    $sql = "select id, nome from cn" . $w;
...
function cadastroUSUARIO($mysqli, $re_sessao, $re, $nome, $estado, $email, $telefone, $cargo, $gestao, $coordenador, $cn, $acesso, $combustivel, $cartao, $data, $hora)
{

    $p = permissaoVerifica($mysqli, "14", $re_sessao); //Cadastra usuário
    $p2 = permissaoVerifica($mysqli, "26", $re_sessao); //Cadastra usuário (BR)
    ...
    if ($acesso === "2" and $email === "") {
        $msg = "Necessário informar o endereço de E-mail do colaborador.";
    } else
    if ($acesso === "2" and argVerifica($mysqli, "email", $email) > 0) {
    ...
    if ($acesso === "2" and $cargo != "43" and (strlen($telefone) != 11 || preg_match('/^([0-9][0-9][1-9][1-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9])$/', $telefone) === 0)) {
        $msg = "O telefone informado é inválido.";
    } else
    if ($acesso === "2" and $cargo === "43" and (strlen($telefone) != 10 || preg_match('/^([0-9][0-9][1-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9])$/', $telefone) === 0)) {
        $msg = "O Telefone do fornecedor informado é inválido.";
    } else
    if ($acesso === "2" and argVerifica($mysqli, "telefone", $telefone) > 0) {
    ...
    if ($gestao === "0") {
        $msg = "Necessário selecionar a gestão do colaborador";
    } else
    if ($acesso === "0") {
    ...
    if ($p2 === 0 and $usuario['re'] != $coordenador) {
        ...
        $sql = "insert into usuario (re, email, telefone, cartao, senha, nome, cn, cargo, gestao, supervisor, sistema, estado, ativo, cadastro, data_cadastro, hora_cadastro) values ('{$re}', '{$email}', '{$telefone}','{$cartao}', '{$senha_bd}', '{$nome}', '{$cn}', '{$cargo}', '{$gestao}', '{$coordenador}', '{$acesso}','{$estado}', '2', '{$re_sessao}','{$data}','{$hora}')";
        if ($mysqli->query($sql)) {
            ...
            enviar_colaborador($colaborador, $body1);
...
{
    $sql = "select u.re as re, u.nome as nome, u.cartao as cartao, u.telefone as telefone, u.email as email, c.nome as coordenador_nome, c.email coordenador_email, resp.nome as resp_nome, resp.email as resp_email, ca.nome as cargo, u.cargo as cargo_id, cn.nome as cn, uf.sigla as uf from usuario u inner join usuario c on c.re=u.supervisor inner join usuario resp on resp.re=u.cadastro inner join cargo ca on ca.id=u.cargo inner join cn on cn.id=u.cn inner join uf on uf.id=cn.uf WHERE u.re='{$re}'";
...
function enviar_colaborador($dados, $body)
{
...
function editaUSUARIO($mysqli, $re_sessao, $re, $nome, $estado, $email, $telefone, $cargo, $supervisor, $gestao, $cn, $acesso, $ativo, $senha, $data, $hora)
{

    $erro = "1";
    $cpCoordenador = "";
    $p = permissaoVerifica($mysqli, "15", $re_sessao); //Usuário edita
    $p2 = permissaoVerifica($mysqli, "27", $re_sessao); //Usuário edita (BR)

    $usuario = $mysqli->query("select u.re as re, u.nome as nome, u.estado as estado, uf.sigla as uf, u.cn as cn, cn.nome as cn1 from usuario u inner join cn on cn.id=u.cn inner join uf on uf.id=u.estado where re='{$re_sessao}'")->fetch_array(MYSQLI_ASSOC);

    if ($p === 0) {
        $msg = "Você não tem permissão para editar usuários.";
    } else
    if ($acesso === "0") {
        $msg = "Necessário selecionar o acesso do colaborador ao sistema.";
    } else
    if ($ativo === "0") {
        $msg = "Necessário selecionar se o colaborador está ativo.";
    } else
    if ($gestao === "0") {
        $msg = "Necessário selecionar a gestão a qual o colaborador pertence.";
    } else
    if ($estado === "0") {
        $msg = "Necessário selecionar o estado(UF).";
    } else
     if ($cn === "0") {
        $msg = "Necessário selecionar o CN.";
    } else
        ...
        $msg = "Necessário informar o nome do colaborador.";
    } else 
    if ($ativo != "2" and (!$telefone || $telefone === "")) {
        $msg = "Necessário informar o telefone do colaborador.";
    } else 
    if ($acesso === "2" and !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $msg = "O e-mail informado é inválido.";
    } else
    ...
    if ($ativo != "2" and (argVerifica($mysqli, $re,  "telefone", $telefone) > 0)) {
        $msg = "Telefone já cadastrado para outro colaborador.";
    } else
    if ($senha <> "" and !preg_match("/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{7,}$/", $senha)) {
        $msg = "A nova senha deve conter 4 caracteres ou mais, números e letras.";
    } else {
        ...
        if ($senha <> "") {
            $senha = md5($senha);
            $sql = "update usuario set email='{$email}', telefone='{$telefone}', senha='{$senha}', nome='{$nome}', estado='{$estado}', cn='{$cn}', cargo='{$cargo}', " . $cpCoordenador . ", sistema='{$acesso}', ativo='{$ativo}', gestao='{$gestao}' where re='{$re}'";
        } else {
            $sql = "update usuario set email='{$email}', telefone='{$telefone}', nome='{$nome}', estado='{$estado}', cn='{$cn}', cargo='{$cargo}', " . $cpCoordenador . ", sistema='{$acesso}', ativo='{$ativo}', gestao='{$gestao}' where re='{$re}'";
        }

        //Armazena dados antes da mudança
        $d = $mysqli->query("select usr.re as re, usr.nome as nome, usr.estado as ufId, uf.sigla as uf, ges.nome as gestao, usr.supervisor as coordenadorRe, co.nome as coordenador, usr.cargo as godCard, c.nome as cargo, usr.email as email, usr.telefone as telefone, usr.cn as cnId, cn.nome as cn, usr.sistema as acessoId, if(usr.sistema=2,'SIM','NAO') as acesso, usr.ativo as statusId, if(usr.ativo=2,'ATIVO','DESATIVADO') as status from usuario usr left join uf on uf.id=usr.estado left join cn on cn.id=usr.cn left join cargo c on c.id=usr.cargo left join usuario co on usr.supervisor=co.re left join gestao ges on ges.id=usr.gestao WHERE usr.re='{$re}'")->fetch_array(MYSQLI_ASSOC);

        if ($mysqli->query($sql)) {

            $historico = array("re" => $re_sessao, "colaborador" => $re, "tipo" => 0, "Txtatual" => "", "Txtnovo" => "", "dh" => DH);

            $d2 = $mysqli->query("select usr.re as re, usr.nome as nome, usr.estado as ufId,uf.sigla as uf, ges.nome as gestao, usr.supervisor as coordenadorRe, co.nome as coordenador, usr.cargo as godCard, c.nome as cargo, usr.email as email, usr.telefone as telefone, usr.cn as cnId, cn.nome as cn, usr.sistema as acessoId, if(usr.sistema=2,'SIM','NAO') as acesso, usr.ativo as statusId, if(usr.ativo=2,'ATIVO','DESATIVADO') as status from usuario usr left join uf on uf.id=usr.estado left join cn on cn.id=usr.cn left join cargo c on c.id=usr.cargo left join usuario co on usr.supervisor=co.re left join gestao ges on ges.id=usr.gestao WHERE usr.re='{$re}'")->fetch_array(MYSQLI_ASSOC);

            if ($d['uf'] != $d2['uf']) {
                $historico['tipo'] = 4;
                $historico['Txtatual'] = $d['uf'];
                $historico['Txtnovo'] = $d2['uf'];
                historico($mysqli, $historico);
            }
            if ($d['cn'] != $d2['cn']) {
                $historico['tipo'] = 5;
                $historico['Txtatual'] = $d['cn'];
                $historico['Txtnovo'] = $d2['cn'];
                historico($mysqli, $historico);
            }
            if ($d['nome'] != $d2['nome']) {
                $historico['tipo'] = 6;
                $historico['Txtatual'] = $d['nome'];
                $historico['Txtnovo'] = $d2['nome'];
                historico($mysqli, $historico);
            }
            if ($d['email'] != $d2['email']) {
                $historico['tipo'] = 7;
            ...
            if ($d['telefone'] != $d2['telefone']) {
                $historico['tipo'] = 8;
                $historico['Txtatual'] = $d['telefone'];
                $historico['Txtnovo'] = $d2['telefone'];
                historico($mysqli, $historico);
            }
            if ($d['godCard'] != $d2['godCard']) {
                $historico['tipo'] = 9;
                $historico['Txtatual'] = $d['godCard'];
                $historico['Txtnovo'] = $d2['godCard'];
                historico($mysqli, $historico);
            }
            if ($d['acessoId'] != $d2['acessoId']) {
                $historico['tipo'] = 11;
                $historico['Txtatual'] = $d['acessoId'];
                $historico['Txtnovo'] = $d2['acessoId'];
                historico($mysqli, $historico);
            }
            if ($d['statusId'] != $d2['statusId']) {
                $historico['tipo'] = 12;
                $historico['Txtatual'] = $d['statusId'];
                $historico['Txtnovo'] = $d2['statusId'];
                historico($mysqli, $historico);
            }
            if ($d['gestao'] != $d2['gestao']) {
                $historico['tipo'] = 16;
                $historico['Txtatual'] = $d['gestao'];
                $historico['Txtnovo'] = $d2['gestao'];
                historico($mysqli, $historico);
            }
            $erro = "0";
...
function transfereColaborador($mysqli, $tipo, $colaborador, $novoCoordenador, $data, $hora)
{
    ...
    if ($tipo === "aceita") {
        ...
        if ($mysqli->query($sql)) {
            ...
            historico($mysqli, $atual['re'], $colaborador, "10", $atual_coordenador, $novo_coordenador, DH);
...
{
    $p = permissaoVerifica($mysqli, "9", $re_sessao); //Veículo cadastra
    ...
    if ($km === "") {
        ...
        if ($mysqli->query($sql)) {
            ...
            if ($mysqli->query($sql)) {
                ...
                historico($mysqli, $re_sessao, $re, "2", $frotaAtual, $frotaNova, DH);
            } else {
...
{
    $erro = "1";
    $p = permissaoVerifica($mysqli, "7", $re_sessao); //CARTÃO USUÁRIO CADASTRA
    ...
    if (strlen($cartaoMotivo) < 10) {
        ...
        if ($mysqli->query($sql)) {
            ...
            if ($mysqli->query("update usuario set cartao='{$cartaoNovo}' where re='{$re}'")) {
                ...
                historico($mysqli, $re_sessao, $re, "1", $cartaoAtual, $cartaoNovo, DH);
                ...
                $erro = "0";
                $msg = "<i class='icon-ok-circle-1'></i> Alteração realizada com sucesso, aguarde o desbloqueio.";
            } else {
...
{
    $erro = "1";
    $p = permissaoVerifica($mysqli, "8", $re_sessao); //CARTÃO USUÁRIO ALTERA (BR)
   ...
   if ($verificaCartao > 0) {
    ...
    if ($cartaoNovo == $cartaoAtual) {
        ...
        if ($mysqli->query("update usuario set cartao='{$cartaoNovo}' where re='{$re}'")) {
            ...
            historico($mysqli, $re_sessao, $re, "2", $cartaoAtual, $cartaoNovo, DH);
...
{
    $erro = "1";
    $p = permissaoVerifica($mysqli, "83", $re_sessao); //CARTÃO SOLICITA DESBLOQUEIO
    ...
    if ($verificaSolicitacao > 0) {
        ...
        if ($mysqli->query("update cartao set status='1', finalidade='1' where controle='{$cartaoAtual}'")) {
            ...
            historico($mysqli, $re_sessao, $re, "15", $cartaoAtual, "", DH);
...
{
    $erro = "1";
    $p = permissaoVerifica($mysqli, "19", $re_sessao); //Cartão remove
    ...
    if ($p === 0) {
        ...
        if ($mysqli->query("update usuario set cartao='' where cartao='{$cartaoAtual}' and re='{$re}'")) {
            ...
            historico($mysqli, $re_sessao, $re, "2", $cartaoAtual, "", DH);
...
{
    $erro = "1";
    $p = permissaoVerifica($mysqli, "17", $re_sessao); //KM altera
    ...
    if ($p === 0) {
        ...
        if ($mysqli->query("update frota set km='{$kmNovo}' where placa='{$frota}'")) {
            ...
            historico($mysqli, $re_sessao, $re, "1", $frota . "->" . $kmAtual, $frota . "->" . $kmNovo, DH);
...
function frotaAtribui($mysqli, $re_sessao, $re, $atual, $novo, $data, $hora)
{
   ...
   if ($verificaFrota > 0) {
        ...
        if ($mysqli->query("update usuario set frota='{$novo}' where re='{$re}'")) {
            ...
            historico($mysqli, $re_sessao, $re, "3", $atual, $novo, DH);
    ...
    $mysqli->close();
    echo json_encode(array("erro" => $erro, "msg" => $msg, "tempo" => $tempo));
}
function frotaRemove($mysqli, $re_sessao, $re, $frotaAtual, $data, $hora)
{
    $erro = "1";
    $p = permissaoVerifica($mysqli, "20", $re_sessao); //VEÍCULO REMOVE
    ...
    if ($p === 0) {
        ...
        if ($mysqli->query("update usuario set frota='' where frota='{$frotaAtual}' and re='{$re}'")) {
            ...
            historico($mysqli, $re_sessao, $re, "3", $frotaAtual, "REMOV", DH);
...
{
    $txt = strtoupper($txt);
    $p = permissaoVerifica($mysqli, "21", $re_sessao); //USUÁRIO VISUALIZA (BR)
    $p2 = permissaoVerifica($mysqli, "64", $re_sessao); //USUÁRIO VISUALIZA (REGIÃO)

    $gestao = regiao($mysqli, $re_sessao)['gestao'];
    ...
    if ($p2 > 0) {
        ...
        $where = " usr.gestao='{$gestao}' and " . $where;
    } else {

        $where = " usr.supervisor='{$re_sessao}' and " . $where;
    }

    $sql = "select usr.id as id, usr.re as re, usr.nome as nome, IF(char_length(usr.frota) = 7 ,usr.frota,'S/VEÍCULO') as veiculo, IF(char_length(usr.cartao) = 6 ,usr.cartao,'S/CARTÃO') as cartao, cor.nome as coordenador, cn.nome as cn, if(usr.ativo=2,'SIM','NÃO') as ativo, per.funcao as permissao from usuario usr inner join cn cn on cn.id=usr.cn inner join usuario cor on cor.re=usr.supervisor left join permissao per on per.colaborador=usr.re WHERE" . $where . " group by usr.re order by cor.nome, usr.nome asc";
...
function historico($mysqli, $data)
{
    $sql = "insert into alteracao (re, re_alterado, tipo, valor_anterior, valor_novo, dh) values ('" . $data['re'] . "', '" . $data['colaborador'] . "', " . $data['tipo'] . ", '" . $data['Txtatual'] . "', '" . $data['Txtnovo'] . "', '" . $data['dh'] . "')";
    $mysqli->query($sql);
}
