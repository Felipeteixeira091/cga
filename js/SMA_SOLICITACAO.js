$(document).ready(function () {
    ...
    $("#formCriar").click(function () {
        ...
        CriaSolicitacao(sigla, tipo, faturaTipo, osTipo, rede, almoxarifado, os, retirada, sobressalente, obs);
    });
                    ...
                    success: function (dados) {
                        ...
                        if (dados.length === 0) {
                            ...
                            for (var i = 0; i < linhas.length; i++) {
                                ...
                                if (sobressalente === linhas[i].sobre) {
                                    lista += "<td>" + linhas[i].sobre + "</td>";
                                    ...
                                    lista += "<td><span class='badge badge-warning'>" + linhas[i].sobre + "</span></td>";
                                    lista += "<td><input class='form-control' disabled id=" + qtd + " value='1' type='number'></td>";
                                    lista += "<td><button disabled class='btAddItem btn btn-sm btn-outline-secondary' value='" + linhas[i].id + "'><i class='icon-minus-circled text-muted'></i> Adicionar</button></td>";
                                }
                                lista += "</tr>";
                            }
                            lista += "</tbody>";
    ...
    function CriaSolicitacao(sigla, tipo, faturaTipo, osTipo, rede, almoxarifado, os, retirada, sobressalente, obs) {
        ...
        $.ajax({
            type: 'post', //Definimos o método HTTP usado
            data: { acao: "criaSolicitacao", sigla: sigla, tipo: tipo, rede: rede, faturaTipo: faturaTipo, osTipo: osTipo, almoxarifado: almoxarifado, os: os, retirada: retirada, sobressalente: sobressalente, obs: obs },
            dataType: 'json', //Definimos o tipo de retorno
            url: 'SMASOLICITACAO', //Definindo o arquivo onde serão buscados os dados
            success: function (dados) {
                if (dados.erro === "1") {
