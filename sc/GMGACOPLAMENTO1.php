...
$acao = "cadastroAC";//$txtTitulo['acao'];
...
if ($acao === "cadastroAC") {
    ...
    $gmg = "eqp3713";//$txtTitulo['gmg'];
    $site = "1577";//$txtTitulo['site'];
    $ta = "12345";//$txtTitulo['ta'];
    $observacao = "TESTE1";//$txtTitulo['observacoes'];
    $data_inicio = "2021-01-12";//$txtTitulo['data_inicio'];
    $hora_inicio = "11:00";//$txtTitulo['hora_inicio'];
    $data_final = "2021-01-12";//$txtTitulo['data_final'];
    $hora_final = "12:00";//$txtTitulo['hora_final'];
...
if ($acao === "ACProcura") {
    ...
    acProcura($mysqli, $cn, $dataInicio, $dataFim, $re);
} else
if ($acao === "ACDetalhe") {
...
{
    $sql = "select g.codigo as id, concat('CN_',cn.nome,' ',gt.nome,'_',g.identificacao) as nome from gmg g inner join gmg_tipo gt on gt.id=g.tipo left join cn on cn.id=g.cn where tipo!='4' and estado='{$uf_sessao}' order by g.cn, gt.id, g.identificacao";
...
function cadastroAC($mysqli, $re, $gmg, $site, $ta, $observacao, $data, $hora, $data_inicio, $hora_inicio, $data_final, $hora_final)
{
    ...
    if ($data_inicio > $data or $data_final > $data) {
     ...
     if ($data_final === $data and ($hora_final > $hora)) {
        ...
        $sql = "insert into gmg_acoplamento (re, gmg, gmg_codigo, site, ta, data, hora, data_inicio, hora_inicio, data_final,hora_final, observacao) values ('{$re}', '0', '{$gmg}', '{$site}','{$ta}','{$data}', '{$hora}', '{$data_inicio}', '{$hora_inicio}', '{$data_final}', '{$hora_final}', '{$observacao}')";
...
function acProcura($mysqli, $cn, $dataInicio, $dataFim, $re)
{
    ...
    $where = $usuario . $where;
    $sql = "select ac.id as id, gmg.identificacao as gmg, site.sigla as site, ac.data as data, ac.hora as hora  from gmg_acoplamento ac inner join gmg gmg on gmg.codigo=ac.gmg_codigo inner join site site on site.id=ac.site WHERE" . $where . " order by data, hora asc";
...
function acDetalhes($mysqli, $id)
{
    ...
    $sql = "select ac.id as id, ac.re as re, usr.nome as nome, coo.re as reCoordenador,coo.nome as nomeCoordenador, gmg.identificacao as gmg, site.sigla as site, cn.nome as cn, ac.data as data, ac.hora as hora, ac.data_inicio as data_inicio, ac.data_final as data_final, ac.hora_inicio as hora_inicio, ac.hora_final as hora_final, ac.observacao as obs from gmg_acoplamento ac inner join gmg gmg on gmg.id=ac.gmg inner join usuario usr on usr.re=ac.re left join usuario coo on coo.re=usr.supervisor inner join site site on site.id=ac.site inner join cn cn on cn.id=site.cn where ac.id='{$id}'";
