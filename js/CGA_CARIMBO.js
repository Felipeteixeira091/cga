$(document).ready(function () {
    ...
    function form_input_os() {
        ...
        $("#input_select_status").change(function () {
            ...
            if (status === "10" || status === "18" || status === "20" || status === "22" || status === "24") {
                $("#input_prediction").val("2022-12-31T23:59");
            } else{
                $("#input_prediction").val("");
            }
    ...
    function stamp_generate(data) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.erro === "1") {
                    var classe = "bg bg-gradient bg-danger text-center rounded fw-bold text-white p-1 m-1";
                    var msg = "<i class='bi bi-exclamation-circle'></i> " + response.msg;

                    $("#modal_return").slideDown("fast").addClass(classe).append(msg).delay(tempo).slideUp("fast");

                } else {
                    var classe = "bg bg-gradient bg-success text-center rounded fw-bold text-white p-1 m-1";
                    var msg = "<i class='bi bi-check-circle'></i> " + response.msg;
        ...
        $("#btn_search_tec").click(function () {
            ...
            $("#tableTec").slideUp("fast", function () {
                ...
                if (txt.length === 0) {
                    ...
                    var tempo = 1500;
                    var classe = "bg-danger rounded font-weight-bold text-white pt-2 pb-2 mt-2 mb-2";
                    ...
                    $.ajax({
                        ...
                        success: function (dados) {
                            ...
                            if (dados.length === 0) {
                                ...
                                lista += "<th scope='col' class='text-center'>TELEFONE</th>";
                                ...
                                for (var i = 0; i < linhas.length; i++) {
                                    ...
                                    lista += "<td class='text-right'><span id='tell" + linhas[i].re + "'><i class='bi bi-telephone-outbound-fill'></i> " + linhas[i].telefone + "</span></td>";
