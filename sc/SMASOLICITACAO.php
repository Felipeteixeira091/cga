...
function SiteProcura($mysqli, $txt, $re_sessao)
{
    ...
    $sql = "SELECT s.id as id, s.sigla as sigla, s.descricao as descricao, st.nome as tipo, cn.nome as cn, uf.sigla as uf FROM site s inner join site_tipo st on st.id=s.tipo inner join cn cn on cn.id=s.cn inner join uf on uf.id=s.estado WHERE cn.regiao='{$regiao}' and s.sigla like '%" . $txt . "%'";
