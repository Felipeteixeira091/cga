...
if ($acao === "alteraSaldo") {
    ...
    $id = $txtTitulo['id'];
    $saldoCorreto = $txtTitulo['novoSaldo'];

    alteraSaldo($mysqli, $id, $saldoCorreto, $data, $hora, $re);
}
function alteraSaldo($mysqli, $id, $saldoCorreto, $data, $hora, $re)
{
    $erro = "1";
    $p = permissaoVerifica($mysqli, "88", $re);

    if ($p === 0) {
        $msg = "<i class='icon-attention'></i> Você não tem permissão.";
    } else {

        if ($mysqli->query("update gas_bagagem set kg='{$saldoCorreto}', data='{$data}', hora='{$hora}' where id='{$id}'")) {


            $erro = "0";
            $msg = "<i class='icon-ok-circle-1'></i> Correção realizada com sucesso.";
        } else {
            $msg = "<i class='icon-attention'></i> Erro ao efetuar correção.";
        }
    }

    $mysqli->close();
    $arr = array("erro" => $erro, "msg" => $msg, "data" => $data, "hora" => $hora);
    echo JsonEncodePAcentos::converter($arr);
}
function saldo($mysqli, $id)
{
    $sql = "select gb.id as id, gt.nome as tipo, gb.kg as kg from gas_bagagem gb inner join gas_tipo gt on gt.id=gb.tipo WHERE gb.id='{$id}'";
...
function scProcura($mysqli, $cn, $nome)
{
    ...
    $sql = "select cn.nome as cn, u.re as re, u.nome as nome, c.re as coRe, c.nome as coNome from gas_tipo gt left join gas_bagagem gb on gb.tipo=gt.id left join usuario u on u.re=gb.re inner join cn on cn.id=u.cn inner join usuario c on c.re=u.supervisor where ".$where." gb.kg>0 group by u.nome order by cn, nome";
//    $sql = "select c.re as re, c.nome as nome, co.re as coRe, co.nome as coNome, cn.nome as cn from usuario c inner join usuario co on co.re=c.supervisor inner join cn on cn.id=c.cn WHERE " . $where . " and c.cargo!= '43' and c.ativo='2' order by nome asc";
...
function lcEstoque($mysqli, $re)
{
    ...
    $sql = "select gb.id as id, gb.re as re, gt.nome as tipo, gb.kg as kg, concat(gb.data,' ',gb.hora) as data from gas_bagagem gb inner join gas_tipo gt on gt.id=gb.tipo where gb.re='{$re}'";
