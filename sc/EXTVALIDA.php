...
if ($acao === "nova") {
    ...
    nova($mysqli, $txtTitulo['fornecedor'], $txtTitulo['tipo'], $txtTitulo['pedido'], $re, date("Y-m-d H:i"));
} else
if ($acao === "update") {
...
function deleta($mysqli, $re, $nota)
{
    ...
    $p = permissao($mysqli, 95, $re);

    if ($p === 0) {
        $msg = "Você não tem permissão para deletar notas.";
    } else {

        $sql = "select na.id as id_anexo, na.arquivo as arquivo from nota_anexo na where na.nota='{$nota}'";
        $arquivos = $mysqli->query($sql)->num_rows;
        $dir = '../nota/';
        ...
        $sql_delete = "delete nota, nota_anexo from nota left join nota_anexo on nota_anexo.nota=nota.id WHERE nota.id='{$nota}'";
        if ($mysqli->query($sql_delete)) {

            $msg = "Foram removidos " . $qtd_arq . " arquivos do servidor e " . ((int)$arquivos + 1) . " registros.";
            $erro = "0";
        } else {
            $msg = "Erro ao remover registros.";
        }
    }
...
{
    $sql = "select u.nome as nome, n.id as id, n.fornecedor as fornecedor, n.tipo as tipo, n.pedido as pedido, n.status as status, concat('Status: ',ifnull(ns.nome,'EM CADASTRO')) as status_span from nota n inner join usuario u on u.re=n.re left join nota_status ns on ns.id=n.status where ";
...
function nova($mysqli, $fornecedor, $tipo, $pedido, $re, $dh)
{
    $sql = "insert into nota (re, fornecedor, tipo, pedido, status, dh) values ('{$re}','{$fornecedor}', '{$tipo}', '{$pedido}', 1, '{$dh}')";
    ...
    if (!$pedido || $pedido === 0 || $pedido === "" || strlen($pedido) <= 3) {
        $msg = "Necessário informar o número do pedido corretamente.";
...
function notaProcura($mysqli, $txt, $data1, $data2, $status, $fornecedor)
{
    ...
    $sql = "select n.id as id, n.pedido as pedido, n.dh as dh, u.nome as nome, s.nome as status, t.nome as tipo, f.nome as fornecedor from nota n inner join nota_status s on s.id=n.status inner join usuario u on u.re=n.re inner join usuario f on f.id=n.fornecedor inner join nota_tipo t on t.id=n.tipo where" . $where . "";
