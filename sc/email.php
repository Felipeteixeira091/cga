...
function enviar($dados, $tabela, $almox)
{
    ...
    if ($almox === "S") {
        ...
        $mail->addCC($dados['almoxEmail'], $dados['almoxNome']);
	...
	//$mail->addBCC('ftsilva@icotel.net', 'Felipe Teixeira');
...
function dados_email($mysqli, $re, $arquivo, $dados)
{
...
$sql = "SELECT usr.email as email, ema_senha as senha, ema_smtp as smtp, usr.nome as nome from email INNER JOIN usuario usr on usr.re=ema_re where usr.re=" . $re;
    ...
    $sql_solicitante = "SELECT u.email as email, u.nome as nome FROM solicitacao s inner join usuario u on u.re=s.re WHERE s.id='{$solicitacao}'";
