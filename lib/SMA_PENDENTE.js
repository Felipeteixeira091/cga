$(document).ready(function () {
    ...
    $("#btFiltra").click(function () {
        ...
        if (!pa) {
            $("#retornoFiltro").slideDown("fast").addClass(classe).append("<i class='icon-attention'></i> Selecionar SIM ou NÃO (Filtro por PA).").delay(1500).slideUp("fast");
        } else
            if (data1 === "" || data2 === "") {
                $("#retornoFiltro").slideDown("fast").addClass(classe).append("<i class='icon-attention'></i> A data inicial e final são obrigatórias.").delay(1500).slideUp("fast");
            } else
                if (txt === "" && status === "0") {
                    $("#retornoFiltro").slideDown("fast").addClass(classe).append("<i class='icon-attention'></i> Informações insuficientes.").delay(1500).slideUp("fast");
                } else {
                    SolicitacaoProcura(txt, status, data1, data2, pa, diffDias);
                }
    });
        ...
        $.ajax({
            ...
            success: function (dados) {
                ...
                if (verifica.length === 0) {
                    ...
                    lista += "<th style='text-align: center' scope='col'><i class='icon-up text-success'></i> SMA</th>";
                    lista += "<th style='text-align: center' scope='col'><i class='icon-down text-danger'></i> SGA</th>";
                    lista += "<th style='text-align: center' scope='col'><i class='icon-info text-info'></i> SALDO</th>";
                    lista += "</tr>";
                    lista += "</thead>";
                    lista += "<tbody>";
                    for (var i = 0; i < linhas.length; i++) {

                        var saldo = linhas[i].sma - linhas[i].sga;
    ...
    function SolicitacaoProcura(txt, status, data1, data2, pa, dias) {

        var delay = 1200;

        $.ajax({
            type: 'post', //Definimos o método HTTP usado
            data: { acao: "SolicitacaoProcura", txt: txt, status: status, data1: data1, data2: data2, pa: pa },
            dataType: 'json', //Definimos o tipo de retorno
                ...
                setTimeout(function () {
                    ...
                    if (verifica.length === 0) {
                        ...
                        $("#retornoFiltro").slideDown("fast").addClass(classe).append(ico + " " + msg).delay(tempo).slideUp("fast", function () {
                            ...
                            lista += "<th scope='col' class='text-center'>SOLICITANTE</th>";
                            lista += "<th scope='col' class='text-center'>SITE</th>";
                            lista += "<th scope='col' class='text-center'>SOBRESSALENTE</th>";
                            ...
                            for (var i = 0; i < linhas.length; i++) {
                                ...
                                var solicitante = linhas[i].solicitante_nome.split(" ");;

                                lista += "<tr id='linha" + linhas[i].id + "'>";
                                lista += "<td>" + linhas[i].id + "</td>";
                                lista += "<td class='text-center'>" + solicitante[0] + " " + solicitante[1] + "</td>";
                                lista += "<td class='text-center'>[" + linhas[i].cn + "] " + linhas[i].site + "</td>";

                                if (linhas[i].sobressalente === "1") {
                                    sobressalente = "SIM";
                                } else {
                                    sobressalente = "NÃO";
                                }
                                lista += "<td class='text-center'>" + sobressalente + "</td>";
                                var data_hora = "";
                                if (linhas[i].hora != "nd") {
                                    data_hora = linhas[i].data + " - " + linhas[i].hora;
                                } else {
                                    data_hora = linhas[i].data;
                                }
                                lista += "<td class='text-center'>" + data_hora + "</td>";
        ...
        $.ajax({
            ...
            success: function (dados) {
                ...
                if (solicitacao.status === "PENDENTE") {
                    ...
                    $("#obs2").hide("fast");
                    status = solicitacao.status + "<i class='icon-ok-circled2 text-success'></i>";
                } else if (solicitacao.status === "EXPIRADO") {

                    $("#formAprova").hide("fast");
                    $("#formReprova").hide("fast");
                    $("#obs2").hide("fast");
                    status = solicitacao.status + "<i class='icon-block text-secondary'></i>";
                    ...
                    status = solicitacao.status + "<i class='icon-cancel-circled-1 text-muted'></i>";
                } else if (solicitacao.status === "PENDENTE") {

                    status = solicitacao.status + "<i class='icon-cart text-warning'></i>";
                    $("#formAprova").hide("fast");
                    $("#formReprova").hide("fast");
                    $("#obs2").hide("fast");
                }
